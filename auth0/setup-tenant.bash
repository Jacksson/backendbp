...
API_NAME=accounts-composite
API_URL=https://localhost:8443/accounts-composite
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST -H "Content-Type: application/json" -d "{\"name\": \"$API_NAME\", \"identifier\": \"$API_URL\", \"scopes\": [{ \"value\": \"customer:read\", \"description\": \"Read customer information\"}, {\"value\": \"customer:write\", \"description\": \"Update customer information\"}]}" https://$TENANT/api/v2/resource-servers | jq .
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$READER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"customer:read\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$WRITER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"customer:read\",\"customer:write\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
